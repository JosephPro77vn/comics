...
router.put('/:id', checkComicOwner, async (req, res)=>{
	...
	try{
		...
		req.flash("success", "Comic updated! Successfuly! ");
		res.redirect(`/comics/${req.params.id}`);
	}catch(err){
		console.log(err);
		req.flash("error", "Error on Updating Comic try again!")
		res.redirect("/comics");
	}
})
...
router.delete("/:id", checkComicOwner, async (req, res) =>{
	...
	try{
		...
		req.flash("success", "Comic deleted Successfuly!")
		res.redirect("/comics");
	}catch(err){
		console.log(err);
		req.flash("error", "Error deleting Comic try again!");
		res.redirect("back");
	}
});
